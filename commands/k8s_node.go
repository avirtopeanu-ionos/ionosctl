...
func k8sNode() *core.Command {
	...
	globalFlags.StringSliceP(config.ArgCols, "", defaultK8sNodeCols, utils.ColsMessage(defaultK8sNodeCols))
...
func getK8sNodePrint(c *core.CommandConfig, k8ss []v5.K8sNode) printer.Result {
	r := printer.Result{}
	if c != nil {
		if k8ss != nil {
			r.OutputJSON = k8ss
...
func getK8sNodes(k8ss v5.K8sNodes) []v5.K8sNode {
	u := make([]v5.K8sNode, 0)
	if items, ok := k8ss.GetItemsOk(); ok && items != nil {
		for _, item := range *items {
			u = append(u, v5.K8sNode{KubernetesNode: item})
		}
	}
	return u
}

func getK8sNode(u *v5.K8sNode) []v5.K8sNode {
	k8ss := make([]v5.K8sNode, 0)
	if u != nil {
		k8ss = append(k8ss, v5.K8sNode{KubernetesNode: u.KubernetesNode})
	}
	return k8ss
}

func getK8sNodesKVMaps(us []v5.K8sNode) []map[string]interface{} {
