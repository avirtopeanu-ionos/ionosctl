...
const (
	...
	stateInProgressStatus = "IN_PROGRESS"
	stateSuccessfulStatus = "SUCCESSFUL"
)
	...
	go func() {
		...
		for {
			...
			case stateActiveStatus:
				...
				errChan <- nil
				return
			case stateInProgressStatus:
				sendingProgress(50)
				break
			case stateSuccessfulStatus:
				sendingProgress(100)
				errChan <- nil
				return
...
func getRequestStatus(reqStatus *v5.RequestStatus) (string, string, error) {
	if reqStatus != nil {
